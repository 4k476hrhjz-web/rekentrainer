...
function launchConfetti(count = 40) {
  ...
  const colors = ['#ff9f45', '#6bcB77', '#4dabf7', '#ffd93d', '#ff6b6b'];
...
// ---------- Home screen ----------
function renderHome() {
  document.getElementById('plant-emoji').textContent = PLANT_EMOJIS[state.plantStage] || '🌱';
...
function showReward(advanced, finishedSession) {
  document.getElementById('reward-plant-emoji').textContent = PLANT_EMOJIS[state.plantStage] || '🌱';
...
function endFlits() {
  ...
  saveState(state);
  document.getElementById('reward-plant-emoji').textContent = '⚡';
